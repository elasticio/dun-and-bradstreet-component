...
public class IndustryState implements Module {
  ...
  public void execute(final ExecutionParameters parameters) {
    ...
    try {
      OrderProductRequest orderProductRequest =
          mapper.readValue(body.toString(), OrderProductRequest.class);

      SOAPMessage response =
          new GenericSOAPClient.Builder()
              .setRequestClass(OrderProductRequest.class)
              .setBodyObject(orderProductRequest)
              .setEndpointUrl(EndpointUrl.INDUSTRY_PRODUCT)
              .setSoapAction(SoapAction.INDUSTRY_PRODUCT)
              .setUsername(Utils.getUsername(configuration))
              .setPassword(Utils.getPassword(configuration))
              .call();

      JAXBElement jaxbElement =
          new GenericSOAPClient.Builder().bindToJaxb(OrderProductResponse.class, response);
      ...
      data =
          (new Message.Builder())
              .body(Json.createObjectBuilder().add("result", e.getMessage()).build())
              .build();
